...
export const zh = {
	...
	'help.nowTrend':
		'当前：最近一个完整的 1 秒窗口内的每秒字节数。折线为页面打开期间最近约一分钟的采样，重置统计或暂停后重新开始。',
	...
	'help.kpi.rate':
		'所有规则在最近一个完整秒内的每秒字节数：↑ 上传为客户端发往目标方向，↓ 下载为返回方向。折线为页面打开期间最近约一分钟的采样，重置统计或暂停后重新开始。',
	...
	ruleNameSlash: '名称不能包含“/”。',
	ruleNameTaken: '已有同名规则。',
	listenAddressTaken: '已被规则“{name}”使用。',
	listenAddressWebUI: '已被网页界面使用。',
	usernameColon: '用户名不能包含“:”。',
	passwordNeedsUsername: '请为该密码设置用户名。',
	hopUrlEmpty: '请输入代理地址。',
	hopUrlNoScheme: '请包含协议（例如 socks5://host:1080）。',
